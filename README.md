...
```text
https://github.com/DDOIT-OFFICIAL/DDOIT_Tools.git#v0.18.24
```
...
{
  "dependencies": {
    "com.ddoit.tools": "https://github.com/DDOIT-OFFICIAL/DDOIT_Tools.git#v0.18.24"
  }
}
```
...
`com.ddoit.tools`의 `package.json`에는 패키지 컴파일에 즉시 필요한 최소 의존성만 둡니다.
Meta XR SDK와 Input System은 UPM 설치 즉시 강제 설치하지 않고, `DDOIT Tools > Setup` 창의 `필수 패키지 설치/업데이트` 버튼에서 설치/검증합니다.

| 구분 | 패키지 | 기준 |
|---|---|---|
| UPM 하드 의존성 | TextMeshPro | `4.0.0+` |
| UPM 하드 의존성 | Addressables | `2.8.1+` |
| Setup 필수 설치/검증 | Meta XR All-in-One SDK | `203.0.0` exact |
...
Meta XR 관련 런타임 스크립트는 `DDOIT_META_XR_AVAILABLE` scripting define이 있을 때만 컴파일됩니다.
Setup 창은 Meta XR 패키지 설치 상태를 감지해 Standalone/Android define을 동기화합니다.
