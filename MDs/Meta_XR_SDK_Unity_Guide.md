...
| **Meta XR Movement SDK** | v201.0.0 | 바디/페이스/아이 트래킹 (GitHub `#v201.0.0`, Min Unity 6 강제) |
...
> **DDOIT 적용 (v0.15.0~v0.16.0 완료)**: 샘플 프로젝트의 3개 핵심 파일(`WalkingStick.cs`, `WalkingStickLocomotor.cs`, `WalkingStickVisual.cs`) + `IsGroundedActiveState.cs` 자체 추가, namespace `Oculus.Interaction.Locomotion` → `DDOIT.Tools.Locomotion`으로 포팅. Sample 알고리즘/상수/AnimationCurve 1:1 미러. ISDK `FirstPersonLocomotor`를 OVRInteractionComprehensive 인스턴스에 prefab modification으로 추가하는 패턴. `WalkingStickNode` 시나리오 노드 + `PlayerRig.EnableWalkingStick()` API 제공. 상세는 `DDOIT_Tools.md` 4.5 섹션 참조.
>
> ⚠️ **DDOIT 통합 디버깅 lessons** (재현 시 누락 금지):
> - `OVRManager._trackingOriginType = FloorLevel` 필수 (EyeLevel 시 CameraRig가 HMD 위치로 jump → tracking-space head.y ≈ 0 → stick 길이 0)
> - PlayerController GameObject `layer = Water` + ISDK CharacterController `_layerMask = 7` (Default+TFX+IgnoreRaycast, Water 제외) — self-cast 방지
> - `TunnelingEffect`의 3개 슬롯(`_leftEyeAnchor`, `_rightEyeAnchor`, `_centerEyeCamera`) 모두 wiring 필요 — prefab 자체는 비어있음
> - HandWalkingStick prefab 인스턴스에 `HmdRef._hmd` ← `OVRHmd.Hmd`, `HandRef._hand` ← `LeftInteractions/RightInteractions.Hand` 외부 wiring 필요
> - `WalkingStickLocomotor.WhenStarted/WhenEnded` UnityEvent에 `FirstPersonLocomotor.set_GroundDamping(5)` / `(40)` wiring (Sample 동일 패턴)
> - OpenXR runtime 환경에서는 `OculusHand_R/L` 비활성, `OpenXRRightHand/OpenXRLeftHand` 활성
...
**핵심 컴포넌트 (Movement SDK v201 기준 실재 확인)**:
...
- `CharacterRetargeter` - Building Block 기반 신규 리타게팅 (Movement SDK v201, **권장**)
- `NetworkCharacterRetargeter` - 네트워크 멀티플레이용 리타게팅 (Movement SDK v201)
- `SkeletonRetargeter` - Low-level Native 리타게팅 (Movement SDK v201)
- `OVRFaceExpressions` - Face Tracking 데이터 (Quest Pro/3)
- `OVREyeGaze` - Eye Tracking 데이터 (Quest Pro)

> **제거된 타입 안내**: `RetargetingAnimationConstraint`는 문서에 과거 언급되었으나 실제 Movement SDK v201(v83 시점 이후)에 **존재하지 않습니다**. `CharacterRetargeter` 또는 `SkeletonRetargeter`를 사용하세요.
...
Multiplayer Building Blocks를 사용한 네트워크 아바타 기능을 지원합니다. Movement SDK v201.0.0(현재 설치본)의 `NetworkCharacterRetargeter` Building Block을 통해 네트워크 아바타 동기화를 보다 쉽게 구현할 수 있습니다.
...
> **v201 기준**: Multiplayer Building Blocks (Photon Fusion / Unity NGO / Shared 변형)과 Movement SDK v201의 `NetworkCharacterRetargeter`를 조합해 네트워크 아바타 동기화를 구현할 수 있습니다.
...
>
> 사용하려면 샘플에서 별도 포팅 필요. **WalkingStick은 DDOIT_Tools v0.15.0~v0.16.0에서 포팅 완료** (`DDOIT.Tools.Locomotion` 네임스페이스, `WalkingStickNode` 시나리오 노드 포함).
...
> **v201 참고**: `NetworkCharacterRetargeter` Building Block은 Movement SDK v201.0.0(현재 프로젝트 설치 버전)에 포함됩니다. Photon Fusion 또는 Unity NGO와 결합해 네트워크 아바타 동기화를 구현할 수 있습니다.
...
> **v201 기준**: `CharacterRetargeter`, `NetworkCharacterRetargeter` Building Block이 Movement SDK v201에 포함됩니다. Retargeting Behavior Flags, Visemes 지원도 v83 시점에 도입되어 v201까지 유지됨.
...
> **v201 기준**: Face Tracking은 ARKitFace, CorrectivesFace 컴포넌트를 사용합니다. Quest 3/3S에서는 Audio-to-Expression(A2E) 방식이 지원됩니다. (Movement SDK v201에서 제공)
...
#### 패턴 2: CharacterRetargeter 기반 Retargeting (Movement SDK v201, 권장)

**목표**: `CharacterRetargeter`를 사용한 현대적(v83 시점 도입, v201까지 유지) Native 리타게팅
...
> **v201 기준**: Movement SDK v201의 `NetworkCharacterRetargeter` Building Block을 사용하거나, Photon Fusion과 직접 통합하여 구현할 수 있습니다.

**필요 컴포넌트**:
- `OVRBody` - Body Tracking 데이터 (로컬 플레이어만)
- `OVRUnityHumanoidSkeletonRetargeter` 또는 `CharacterRetargeter` - 리타게터
- `NetworkCharacterRetargeter` (Movement SDK v201) 또는 Photon Fusion NetworkTransform
...
> **문서 작성일**: 2026년 3월 (v83.0.4 기준으로 작성) / **업데이트 이력**: 2026-04-22 (v85.0.0 반영) / 2026-04-28 (v201.0.0 반영, Unity OpenXR 1.17.0 권장 + Meta XR Feature targetApiVersion 알려진 이슈 추가, **Movement SDK v201.0.0 반영**) / 2026-05-04 (Movement SDK v201 본문 잔존 표기 일괄 정리 + WalkingStick 포팅 완료 표기 + DDOIT 통합 디버깅 lessons 반영)
