...
> **기준 SDK 버전**: Meta XR All-in-One SDK v201.0.0 (Core / Interaction / Platform / Haptics / MRUK)
> **부분 미출시**: Audio SDK v85.0.0 / Voice SDK v85.0.0 / Movement SDK v83.0.0 — 모두 v201 Core와 호환
>
...
- Meta XR SDK v74.0 이상 (현재 프로젝트: Core/Interaction v201.0.0 + Movement v83.0.0)
- Horizon OS v60.0 이상 (현재 프로젝트 타겟: v201 / Horizon OS 2.1+)
- Movement SDK는 v85+ 미출시로 **v83.0.0 유지**. Core SDK v201과 공존 호환 확인됨.

**v83 Movement SDK의 누적 강화 사항** (v201 시점에도 유효, 실제 코드 검증 완료):
- `CharacterRetargeter` (네임스페이스: `Meta.XR.Movement.Retargeting`) — Upper body 전용 모드 + Full body 시 leg scaling 옵션
- `HandSkeletalProcessor` (`TargetProcessors/HandSkeletalProcessor.cs`) — 커스텀 핸드 처리 (IK 기능 포함)
- `TwistSkeletalProcessor` (`TargetProcessors/TwistSkeletalProcessor.cs`) — 팔·다리 twist joint 자동 매핑
- `RetargetingBehaviorFlags` enum (`MSDKUtility.cs`) — `ApplyJointOrientationFixup` / `UseTPoseForJointScale` / `ApplyMSDKSourceHandBugFixup` 플래그 조합 가능
- `IK/IKUtilities.cs` — IK 보조 유틸리티
- 자동 매핑/정렬/조인트 인식 알고리즘 개선
...
```
1. Hierarchy에서 우클릭 → Meta > Movement > CharacterRetargeter 블록 추가
   (또는 Meta > Tools > Building Blocks 창에서 CharacterRetargeter 드래그)
2. 아바타를 Character로 지정 (Humanoid Animator 연결)
3. MetaSourceDataProvider가 OVRBody 데이터를 자동 감지
```
...
> **문서 작성일**: 2026년 3월 (v83.0.4 기준으로 작성) / **업데이트 이력**: 2026-04-22 (v85.0.0 반영) / 2026-04-28 (v201.0.0 반영, Unity OpenXR 1.17.0 권장 + Meta XR Feature targetApiVersion 알려진 이슈 추가)
> **기준 SDK 버전**: Meta XR All-in-One SDK v201.0.0 (Core/Interaction/Platform/Haptics/MRUK)
> **부분 미출시**: Audio v85.0.0, Voice v85.0.0, Movement v83.0.0 (모두 v201 Core와 호환)
