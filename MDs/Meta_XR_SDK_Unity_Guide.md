...
| Core / Interaction / Interaction OVR / Platform / Haptics / MRUK | 203.0.0 |
| Audio / Voice | 85.0.0 |
...
중요: Audio/Voice가 85.0.0인 것은 누락이 아니다. All-in-One 203.0.0의 실제 dependency 구성이 그렇다.
...
```text
Library/PackageCache/com.meta.xr.sdk.all@d1f4c6eeec90
Library/PackageCache/com.meta.xr.sdk.core@f4a2a776f923
Library/PackageCache/com.meta.xr.sdk.interaction@90e9cc9754fc
Library/PackageCache/com.meta.xr.sdk.interaction.ovr@3486ee7ba1c5
Library/PackageCache/com.meta.xr.mrutilitykit@0aaed07a91ec
Library/PackageCache/com.meta.xr.sdk.platform@2d05b5b31e56
Library/PackageCache/com.meta.xr.sdk.haptics@545ac4338475
Library/PackageCache/com.meta.xr.sdk.audio@baafb66e77db
Library/PackageCache/com.meta.xr.sdk.voice@f4180b2de811
Library/PackageCache/com.meta.xr.sdk.movement@625f59cb9e35
```
...
현재 manifest는 Unity-Movement GitHub main을 추적한다.

```json
"com.meta.xr.sdk.movement": "https://github.com/oculus-samples/Unity-Movement.git"
```

현재 lock hash:

```text
625f59cb9e350d4baf0c80f8debcc370266c80b9
```

이 hash는 현재 GitHub `HEAD`와 일치하지만 `v203.0.0` tag hash와는 다르다. 재현성이 중요한 작업에서는 아래 tag pin을 먼저 검토한다.

재현성을 우선하는 템플릿/배포 프로젝트에서는 tag pin을 검토한다.

```json
"com.meta.xr.sdk.movement": "https://github.com/oculus-samples/Unity-Movement.git#v203.0.0"
```

단, tag pin 변경은 별도 작업으로 처리하고 Unity compile/Movement sample 영향까지 확인한다.
...
2026-07-09 기준으로 `Library/PackageCache`의 Meta 관련 패키지 전체 `.cs` 파일을 판독 대상으로 삼았다. 판독은 모든 파일을 실제로 열어 파일 수, 라인 수, 타입 선언, 네임스페이스, `MonoBehaviour`, `ScriptableObject`, `MenuItem`, `CreateAssetMenu`, `Obsolete`, Building Block 관련 표면을 추출하는 방식으로 수행했다. 그 다음 구현 판단에 직접 쓰이는 핵심 런타임/에디터 파일은 별도로 수동 확인했다.

| PackageCache package | Version | `.cs` | Lines | Type decl. | Content hash |
|---|---:|---:|---:|---:|---|
| `com.meta.xr.mrutilitykit@0aaed07a91ec` | 203.0.0 | 128 | 43,450 | 254 | `4AF41B9544920778` |
| `com.meta.xr.sdk.all@d1f4c6eeec90` | 203.0.0 | 0 | 0 | 0 | - |
| `com.meta.xr.sdk.audio@baafb66e77db` | 85.0.0 | 34 | 12,640 | 81 | `491A8F30B0EE3748` |
| `com.meta.xr.sdk.core@f4a2a776f923` | 203.0.0 | 1,014 | 257,295 | 2,725 | `2BB8B324C91F0893` |
| `com.meta.xr.sdk.haptics@545ac4338475` | 203.0.0 | 12 | 2,352 | 17 | `B6363A071B711B59` |
| `com.meta.xr.sdk.interaction.ovr@3486ee7ba1c5` | 203.0.0 | 63 | 7,965 | 99 | `0724C1A59250D4F4` |
| `com.meta.xr.sdk.interaction@90e9cc9754fc` | 203.0.0 | 692 | 119,746 | 1,326 | `00EAA0B242090A81` |
| `com.meta.xr.sdk.movement@625f59cb9e35` | 203.0.0 | 167 | 48,273 | 422 | `176C59EEC3834093` |
| `com.meta.xr.sdk.platform@2d05b5b31e56` | 203.0.0 | 234 | 23,167 | 349 | `3BEBE1EE24660E03` |
| `com.meta.xr.sdk.voice@f4180b2de811` | 85.0.0 | 671 | 89,149 | 848 | `F4FC49E2BDC0AE26` |
| **Total** | - | **3,015** | **604,037** | **6,121** | - |
...
|---|---:|---:|---:|---:|---:|---:|---:|
| MRUK | 33 | 23 | 3 | 58 | 1 | 1 | 14 |
| Audio | 10 | 15 | 7 | 0 | 2 | 2 | 0 |
| Core | 144 | 492 | 24 | 57 | 19 | 10 | 260 |
| Haptics | 4 | 2 | 1 | 0 | 0 | 0 | 0 |
| Interaction OVR | 29 | 25 | 0 | 6 | 1 | 0 | 17 |
| Interaction | 320 | 118 | 13 | 61 | 18 | 13 | 0 |
| Movement | 40 | 60 | 4 | 0 | 4 | 0 | 4 |
| Platform | 9 | 9 | 2 | 4 | 1 | 0 | 2 |
| Voice | 82 | 163 | 17 | 16 | 6 | 2 | 0 |
...
- Movement: `GameObject/Movement SDK/Body Tracking/Add Character Retargeter`, `GameObject/Movement SDK/Networking/Add Networked Character Retargeter`, `Assets/Movement SDK/Body Tracking/Run Default Retargeting Setup`, `Meta/Samples/Build Movement SDK Samples`
...
```powershell
Get-Content -LiteralPath "Library\PackageCache\com.meta.xr.sdk.core@f4a2a776f923\Scripts\OVRManager.cs" -Encoding UTF8 -TotalCount 200
```
...
Editor automation이 필요하면 `MSDKUtilityEditor.RunDefaultRetargetingSetup(...)`를 검토한다.

```powershell
rg --line-number "RunDefaultRetargetingSetup|CharacterRetargeter|MetaSourceDataProvider" "Library\PackageCache\com.meta.xr.sdk.movement@*"
```
...
```text
Library/PackageCache/com.meta.xr.sdk.core@.../Editor/RuntimeOptimizer/RuntimeOptimizer.cs
```
