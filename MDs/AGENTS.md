...
이 파일은 AI Agent와 함께 작업할 때 사용하는 프로젝트 지침입니다.
...
## DDOIT_Tools 개발 원칙
- 이 프로젝트(`DDOIT_Template`)는 `DDOIT_Tools` UPM 패키지의 원본 개발 프로젝트입니다.
...
- 반대 방향 수정은 사용자가 명시하지 않는 한 수행하지 않습니다.
- UPM 패키지 소비 프로젝트에서는 `Packages/com.ddoit.tools`를 읽기 전용 외부 패키지로 취급하고, 공통 모듈 개선은 이 프로젝트로 가져와 진행합니다.
...
AI Agent 대화에서 `unity-cli` 명령으로 Unity 에디터의 상태 조회, C# 코드 실행, 플레이 모드 제어 등이 가능합니다.

현재 확인된 CLI/Connector 버전:
- CLI: 0.3.22
- Connector: 0.3.22
...
unity-cli console --type error,warning --lines 20      # 최근 에러/경고 20개
unity-cli console --stacktrace none                    # 첫 줄만
unity-cli console --stacktrace user                    # 사용자 코드 중심 스택트레이스
unity-cli console --stacktrace full                    # 전체 스택트레이스
...
#### 출력 반환
`exec`는 출력이 필요하면 명시적으로 `return`을 사용합니다.

```bash
unity-cli exec "return EditorBuildSettings.scenes.Length;"
# → 3
```
...
```bash
unity-cli exec "return World.DefaultGameObjectInjectionWorld.EntityManager.EntityCount;" --usings Unity.Entities
```
...
```bash
# Build Settings 씬 목록
unity-cli exec "return string.Join(\", \", EditorBuildSettings.scenes.Select(s => System.IO.Path.GetFileNameWithoutExtension(s.path)));"
...
# 특정 오브젝트의 컴포넌트 목록
unity-cli exec "return string.Join(\", \", GameObject.Find(\"ScenarioManager\").GetComponents<Component>().Select(c => c.GetType().Name));"
```
